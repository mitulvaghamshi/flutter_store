...
void main() {
  ...
  test('Root', () async {
    final response = await http.get(.parse(host));
    expect(response.statusCode, 200);
    expect(response.body, '<h2>Pet API v1.0</h2>\n');
  });

  test('404', () async {
    final response = await http.get(.parse('$host/foobar'));
    expect(response.statusCode, 404);
  });

  test('Get record at id: 1', () async {
    final response = await http.get(.parse('$host/pets/1'));
    expect(response.statusCode, 200);
    expect(
      response.body,
  ...
  test('Search record with: "dog"', () async {
    final response = await http.get(.parse('$host/pets/search/dog'));
    expect(response.statusCode, 200);
    expect(
      response.body,
      '[{"id":1,"animal":"Dog","description":'
