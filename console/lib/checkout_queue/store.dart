import 'package:console/checkout_queue/checkout_line.dart';
import 'package:console/checkout_queue/config.dart';
import 'package:console/checkout_queue/customer.dart';
import 'package:console/checkout_queue/linked_queue.dart';
...
class Store {
  ...
  late final _checkoutLines = List.generate(
    config.totalLines,
    (_) => CheckoutLine<Customer>(),
  );
}
...
extension Utils on Store {
  ...
  void populateStore(Iterator<String> entries) {
    ...
    while (i < config.totalCustomer && entries.moveNext()) {
      var customer = Customer(id: i + 1, cartSize: .parse(entries.current));
