...
import 'package:console/elevation_peak/peak.dart';
...
class Elevation {
  const Elevation({required this.config});
...
extension Problem1 on Elevation {
...
extension Problem2 on Elevation {
...
extension Problem3 on Elevation {
...
extension Problem4 on Elevation {
  Pair<int> findMostCommonHeight(List<List<int>> data) {
    ...
    for (var row in data) {
      for (var item in row) {
        elevationCount.update(item, (value) => ++value, ifAbsent: () => 1);
      }
    }
