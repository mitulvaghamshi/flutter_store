...
import 'package:console/elevation_peak/config.dart';
import 'package:console/elevation_peak/elevation.dart';

void main() async {
  print('*** Welcome to the town of Hogsface in the magical land of Foon ***');

  final config = await getConfigFromFile();
  ...
  print(
    ...
    'Lowest elevation: ${minElevation.a}, '
    'and it occurs: ${minElevation.b} times.\n'
    'Total number of peaks: ${localPeaks.length}.\n'
    'Distance between two closest peaks: '
    '${closestPeaks.a.distanceTo(closestPeaks.b).toStringAsFixed(2)} m,\n'
...
Future<Config> getConfigFromFile() async {
  final file = File.fromUri(.file('data.txt'));
  final lines = (await file.readAsLines()).iterator;
  if (!lines.moveNext()) throw FileSystemException('Invalid file!');
